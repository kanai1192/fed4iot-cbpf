...
def createTVs():
    ...
    for i in range(TV_NUM):
        ...
        if (deploy_method == 'docker'):
            ...
            ARG = ['python3', 'f4i.py', 'add-thingvisor', '-c', Master_Controller, '-i', TV_NAME, '-n', tv_name, '-d', DESC, '-p', PARAM]
...
def createvSilos():
    ...
    for i in range(SILO_NUM):
        ...
        if (deploy_method == 'docker'):
            ARG = ['python3', 'f4i.py', 'create-vsilo', '-c', Master_Controller, '-f', FLAV_NAME, '-t', tenant, '-s', vSilo_name]

        if (deploy_method == 'k8s'):
            ARG = ['python3', 'f4i.py', 'create-vsilo', '-c', Master_Controller, '-f', FLAV_NAME, '-t', tenant, '-s', vSilo_name, '-z', FLAV_ZONE]
