...
pid = os.getpid()
log_name = './logs/consumer_mobius_'+str(pid)+'.csv'
print (log_name)
####
...
def on_receive(jres):
    ...
    try:
        if 'm2m:sgn' in jres:
            ...
            print ('write log {}'.format(log_name))

            # added by kenji for saving log
            with open(log_name, 'a') as f:
                writer = csv.writer(f)
                writer.writerow([str(log_timestamp), 'mobius', msg_num, delta_timestamp, total_timestamp/samples]) 
            ######
